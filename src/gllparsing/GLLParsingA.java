...
public class GLLParsingA {
	...
	public static void main(String []args) {
		...
		File fdump=new File("dumplin1.txt");
		Scanner buffer;
		if(f.exists()){
			try{
	...
	private static String parse(char []buf){
		...
		op.add(OperazioneLineare.creaInsertNodeGSS("$"));
		op.add(OperazioneLineare.creaInsertNodeGSS("Ls"+i+"L0"));
		op.add(OperazioneLineare.creaInsertEdgeGSS(gss.getLastNode().element(),u0.element()));
		op.add(OperazioneLineare.creaInsertNodeSppf(cn.toString(),"S"));
	...
	private static Vertex<IdNodoSppf> getNodeT(String simbolo,String item,Vertex<IdNodoSppf>cn){
		...
		if((flag)&&(cn.element().getNomeNodo().equals(nonTerm))) {
			...
			op.add(OperazioneLineare.creaInsertNodeSppf(v.toString(),item));
			sppf.insertDirectedEdge(cn, v, null);
			op.add(OperazioneLineare.creaInsertEdgeSppf(cn.toString(),v.toString()));
			return v;
		}
		return cn;
	}
	...
	private static void add(String etichetta, Vertex<String> nu,int j,Vertex<IdNodoSppf>cn){
		...
		op.add(OperazioneLineare.creaInsertUelement(etichetta, nu.element()));
		op.add(OperazioneLineare.creaInsertRelement(etichetta,nu.element(), j,cn.toString()));
	}
	...
	private static Vertex<String> create(String etichetta,Vertex<String> u,int j,Vertex<IdNodoSppf>cn){
		...
		if(v==null){
			...
			op.add(OperazioneLineare.creaInsertNodeGSS(v.element()));
		}
		...
		if((flag)&&(!(v.element().equals(u.element())))){
			...
			op.add(OperazioneLineare.creaInsertEdgeGSS(v.element() ,u.element()));
			for(ElementoP elp:p){
	...
	private static boolean first(char x,String handle){
		...
		if((handle.equals("Sd"))&&((x=='d')||(x=='a')||(x=='b'))){return true;}
